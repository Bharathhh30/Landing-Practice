    ...
    <header className='fixed top-0 left-0 w-full z-50 py-10'>
      <div className='container flex h-14 items-center max-lg:px-5'>
        <a className='lg:hidden flex-1 cursor-pointer z-2'>
          <img src="/images/xora.svg" width={155} height={55} alt="" />
        </a>
        ...
        <div className={clsx('w-full max-lg:fixed max-lg:top-0 max-lg:left-0 max-lg:w-full max-lg:bg-s2 max-lg:opacity-0', open ? "max-lg:opacity-100" : "max-lg:pointer-events-none")}>
          <div className=' max-lg:relative max-lg:flex max-lg:min-h-screen max-lg:flex-col max-lg:p-6 max-lg:overflow-hidden sidebar-before max-md:px-4'>
        ...
        <button className='lg:hidden z-2 size-10 border-2 border-s4/25 rounded-full flex justify-center items-center'
          onClick={()=>setOpen((prev) => (!prev))} 
           >
          <img src={`/images/${open ? "close" : "magic"}.svg`} alt="magic"  className='size-1/2 object-contain'/>
        </button>
      </div>
    </header>
  ) 
